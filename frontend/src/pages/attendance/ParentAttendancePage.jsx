...
export default function ParentAttendancePage() {
  ...
  return (
    <div className="space-y-6 transition-colors">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Child Attendance</h1>
        <p className="mt-1 text-slate-600 dark:text-slate-300">View read-only attendance for your linked child.</p>
      </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          ...
          <div className="mt-6 space-y-2">
            {attendance.history.map((record) => (
              <div key={record.id} className="flex justify-between rounded-lg bg-slate-50 px-4 py-2 text-sm dark:bg-slate-800">
                <span>
